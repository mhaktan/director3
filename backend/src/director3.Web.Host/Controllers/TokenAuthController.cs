...
using director3.Authentication;
using director3.Entities;
using director3.EntityFrameworkCore;

namespace director3.Web.Host.Controllers
{
        ...
        public ActionResult Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                ...
                if (user.PasswordHash != null && user.PasswordHash.StartsWith("PBKDF2$"))
                    pwOk = director3.Users.PasswordHelper.VerifyPbkdf2(model.Password, user.PasswordHash);
                else
