...
export const DashboardScreen: React.FC = () => {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/PurchaseRequest/GetAll?StatusIn=1%2C2&MaxResultCount=1`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
  ...
  const [count_purchaseRequest_2Data, setCount_purchaseRequest_2Data] = React.useState<Record<string, unknown> | null>(null);
  const [count_purchaseRequest_2Loading, setCount_purchaseRequest_2Loading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/services/app/PurchaseRequest/GetAll?Status=3&MaxResultCount=1`, { headers: getRequestHeaders() });
        // Token expired or invalid — clear auth and redirect to login
        if (res.status === 401) {
          ['_auth_token', '_bearer_token', '_refresh_token'].forEach(k => localStorage.removeItem(k));
          if (window.location.pathname !== '/login') window.location.href = '/login';
        ...
        // Auto-unwrap ABP/generic envelopes so {result.totalCount} or bare {totalCount} both work
        const json = unwrapResponse(rawJson) as Record<string, unknown>;
        setCount_purchaseRequest_2Data(json);
      } catch { /* ignore */ }
      finally { setCount_purchaseRequest_2Loading(false); }
    };
    fetchData();
  }, []);
  ...
  const [breakdown_purchaseOrder_5Data, setBreakdown_purchaseOrder_5Data] = React.useState<Record<string, unknown>[]>([]);
  const [breakdown_purchaseOrder_5Loading, setBreakdown_purchaseOrder_5Loading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const json = unwrapResponse(rawJson) as Record<string, unknown>;
        const target = getNestedValue(rawJson as Record<string, unknown>, 'result') ?? getNestedValue(json, 'result');
        setBreakdown_purchaseOrder_5Data(extractArray(target ?? json));
      } catch { /* ignore */ }
      finally { setBreakdown_purchaseOrder_5Loading(false); }
    };
    fetchData();
  }, []);

  const [list_purchaseRequest_6_tableData, setList_purchaseRequest_6_tableData] = React.useState<Record<string, unknown> | null>(null);
  const [list_purchaseRequest_6_tableLoading, setList_purchaseRequest_6_tableLoading] = React.useState(true);
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setList_purchaseRequest_6_tableData(target != null ? (target as Record<string, unknown>) : json);
      } catch { /* ignore */ }
      finally { setList_purchaseRequest_6_tableLoading(false); }
    };
    fetchData();
  }, []);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 16 }}>
        <div style={{ gridColumn: 'span 12' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            ...
            <div style={{ gridColumn: 'span 1' }}>
              ...
              <UiCard bodyStyle={{ padding: 20 }}>
                <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Onay Bekleyen Talepler</div>
            ...
            <div style={{ gridColumn: 'span 1' }}>
              {count_purchaseRequest_2Loading ? (
                <UiCard bodyStyle={{ padding: 20 }}>
                  <div style={{ height: 12, width: '40%', background: '#e0e0e0', borderRadius: 4, marginBottom: 12, animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <div style={{ height: 28, width: '60%', background: '#e0e0e0', borderRadius: 4, marginBottom: 8, animation: 'pulse 1.5s ease-in-out infinite' }} />
                  <div style={{ height: 10, width: '50%', background: '#f0f0f0', borderRadius: 4, animation: 'pulse 1.5s ease-in-out infinite' }} />
              ...
              <UiCard bodyStyle={{ padding: 20 }}>
                <div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Onaylanmış / Siparişe Hazır</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#ff9800' }}>{(getNestedValue(count_purchaseRequest_2Data ?? {}, 'result.totalCount') as string | number) ?? '—'}</div>
              </UiCard>
              )}
        ...
        <div style={{ gridColumn: 'span 4' }}>
          {breakdown_purchaseOrder_5Loading ? (
            <UiCard header="Sipariş Durum Dağılımı" bodyStyle={{ padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', height: 280 }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 32, height: 32, border: '3px solid #e0e0e0', borderTopColor: '#1976d2', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                <span style={{ fontSize: 12, color: '#999' }}>Loading...</span>
              </div>
          ...
          <UiCard header="Sipariş Durum Dağılımı" bodyStyle={{ padding: 16 }}>
            <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={breakdown_purchaseOrder_5Data}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="label" />
                    <YAxis />
                    <Tooltip />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(1, 1fr)', gap: 16 }}>
            <div style={{ gridColumn: 'span 1' }}>
              <div style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e8e8e8' }}>
                {list_purchaseRequest_6_tableLoading ? (
                  <div style={{ padding: 20, textAlign: 'center', color: '#999' }}>Loading...</div>
                ) : (
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Request Number</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Needed Date</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Status</th><th style={{ padding: '8px 12px', textAlign: 'left', borderBottom: '2px solid #e0e0e0', fontSize: 12, color: '#666' }}>Total Amount</th></tr></thead>
                    <tbody>
                      {(Array.isArray(list_purchaseRequest_6_tableData) ? list_purchaseRequest_6_tableData : extractArray(list_purchaseRequest_6_tableData)).map((row: Record<string, unknown>, i: number) => (
                        <tr key={i}><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['requestNumber'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['neededDate'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['status'] ?? '')}</td><td style={{ padding: '8px 12px', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}>{String(row['totalAmount'] ?? '')}</td></tr>
                      ))}
                    </tbody>
                  </table>
                )}
